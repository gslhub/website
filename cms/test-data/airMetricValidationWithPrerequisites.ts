...
import { generateAIRMetricValidationRecords } from './airMetricValidationBatch';
...
const findDefinitionCount = async ({
  payload,
  req,
}: {
  payload: Payload;
...
export const generateAIRMetricValidationWithPrerequisites = async ({
  payload,
  req,
  batchCode,
}: {
  ...
  const { present, missing } = await findDefinitionCount({ payload, req });
  ...
  try {
    const airRecords = (await generateAIRMetricValidationRecords({
      payload,
      req,
      batchCode,
    })) as GeneratedRecord[];

    return [...createdDefinitions, ...airRecords];
  } catch (error) {
