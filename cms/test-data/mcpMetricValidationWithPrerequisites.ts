...
import { generateMCPMetricValidationRecords } from './mcpMetricValidationBatch';
...
export const generateMCPMetricValidationWithPrerequisites = async ({
  payload,
  req,
  batchCode,
}: {
  ...
  try {
    const mcpRecords = (await generateMCPMetricValidationRecords({
      payload,
      req,
      batchCode,
    })) as GeneratedRecord[];

    return [...createdDefinitions, ...mcpRecords];
  } catch (error) {
