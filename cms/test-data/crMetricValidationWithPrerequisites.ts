...
import { generateCRMetricValidationRecords } from './crMetricValidationBatch';
...
export const generateCRMetricValidationWithPrerequisites = async ({
  payload,
  req,
  batchCode,
}: {
  ...
  try {
    const crRecords = (await generateCRMetricValidationRecords({
      payload,
      req,
      batchCode,
    })) as GeneratedRecord[];

    return [...createdDefinitions, ...crRecords];
  } catch (error) {
