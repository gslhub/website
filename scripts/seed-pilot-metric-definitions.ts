  ...
  },
  {
    ...
    requiredInputs: [
      ...
      },
      {
        ...
        required: true,
        description:
          'Only accepted observations contribute to the mean.',
      },
    ],
    en: {
...
const seedPilotMetricDefinitions = async () => {
  const payload = await getPayload({ config });

  payload.logger.info('Resolving GSLHub pilot metric-definition context...');
  ...
  for (const metric of METRICS) {
    ...
    if (existing.docs.length > 0) {
      skipped += 1;
      payload.logger.info(
        `${metric.definitionCode} already exists — skipped without modification.`,
      );
      continue;
    }
    ...
    const sharedData = {
      ...
      requiredInputs: metric.requiredInputs,
      project: project.id,
    ...
    created += 1;
    payload.logger.info(
      `${metric.definitionCode} (${metric.metricCode} ${PILOT.version}) created in English and Spanish as Under review / Draft.`,
    );
  }

  payload.logger.info(
    `Pilot metric-definition seed complete: ${created} created, ${skipped} already present.`,
  );
};

try {
  await seedPilotMetricDefinitions();
  process.exit(0);
} catch (error) {
  console.error('Unable to seed pilot metric definitions.', error);
  process.exit(1);
}
